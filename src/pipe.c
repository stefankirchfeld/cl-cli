...
int pipe_mode(const Config *config, const char *context,
              const char *question, int freetext) {
    /* read all stdin */
    ...
    if (!prompt) { free(stdin_buf); return 1; }

    int off = 0;
    if (context && context[0])
        off += snprintf(prompt + off, prompt_cap - (size_t)off,
                        "Context:\n%s\n", context);
    if (len > 0)
        off += snprintf(prompt + off, prompt_cap - (size_t)off,
                        "Input:\n%s\n", stdin_buf);
    if (question && question[0])
        snprintf(prompt + off, prompt_cap - (size_t)off,
                 "Question: %s", question);
    ...
    if (resp->type == CL_RESP_COMPLETIONS) {
        if (resp->nlines > 0)
            fputs(resp->lines[0], stdout);  /* no trailing newline — safe in readline */
