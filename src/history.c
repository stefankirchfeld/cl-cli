...
History* history_load(void) {
  ...
  while (fgets(line, sizeof(line), f) && h->count < CL_HISTORY_MAX) {
    line[strcspn(line, "\r\n")] = '\0';
    if (line[0]) h->entries[h->count++] = strdup(line);
  }
  fclose(f);
  return h;
}
...
void history_append(History* h, const char* entry) {
  ...
  if (h->count > 0 && str_equals(h->entries[h->count - 1], entry)) return;

  char* entry_to_add = strdup(entry);
  if (h->count < CL_HISTORY_MAX) {
    h->entries[h->count++] = entry_to_add;
  } else {
    /* shift out oldest */
    free(h->entries[0]);
    memmove(h->entries, h->entries + 1, (CL_HISTORY_MAX - 1) * sizeof(char*));
    h->entries[CL_HISTORY_MAX - 1] = entry_to_add;
  }
  ...
  FILE* f = fopen(path, "w");
  if (!f) return;
  for (int i = 0; i < h->count; i++) fprintf(f, "%s\n", h->entries[i]);
  fclose(f);
}
