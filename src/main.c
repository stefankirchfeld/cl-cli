...
int main(int argc, char **argv) {
    ...
    if (argc > 1 && (strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0)) {
        puts(
            ...
            "\n"
            "Modes:\n"
            "  cl                     Interactive TUI — type a query, get 3 shell completions\n"
            "  cl <prompt>            Interactive TUI pre-populated with <prompt>\n"
            "  <cmd> | cl <question>  Pipe mode — send stdin + question to Claude, print answer\n"
            "\n"
            "Note: completions are printed to stdout. Only the Ctrl+K / Ctrl+F bindings\n"
            ...
            "  Ctrl+K      Open interactive TUI (picks up current readline line as prompt)\n"
            "  Ctrl+F      Same, but allows free-text responses\n"
            "\n"
            "Interactive TUI keys:\n"
            "  Type        Enter your query (debounce triggers Claude after 1s pause)\n"
            "  Up/Down     Browse history (no completions) or select completion\n"
            "  Enter       Confirm selected completion — lands in your shell readline buffer\n"
            "  Ctrl+C      Cancel\n"
            "\n"
            "Examples:\n"
