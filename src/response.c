...
const char CL_SYSTEM_PROMPT_STRICT[] =
    ...
    "\n"
    "The user types potentially mixed input: partial commands, typos, natural language "
    "hints, or a combination of all three. Your job is to apply judgement and infer "
    "what valid shell command(s) the user is most likely trying to run, then produce "
    "the top 3 completions that make sense given that input.\n"
    "\n"
    "In this mode your job is NOT to reply with free text, explanations, or questions. "
    "You ONLY generate completions. Even if the input is ambiguous or partially "
    "natural language, always translate it into the 3 most plausible shell commands.\n"
    "\n"
    ...
    "<third completion>\n"
    "\n"
    "- Start with the exact word COMPLETIONS on its own line, nothing before it.\n"
    "- Exactly 3 completions, one per line, no blank lines between them.\n"
    "- Each completion is a single valid ready-to-run shell command, no numbering, "
...
const char CL_SYSTEM_PROMPT_FREETEXT[] =
    ...
    "You help users with shell commands, command completion, and terminal questions.\n"
    "\n"
    "You MUST respond in exactly one of two formats — no exceptions, no markdown "
...
ClResponse *cl_response_parse(const char *raw) {
    ...
    if (!raw || !*raw) { r->type = CL_RESP_ERROR; return r; }
